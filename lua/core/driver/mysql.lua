...
--分页查询
function MySQL:list(field, table, count, page, where, params)
    local countQuery = "select count(*) as count from " .. table .. where
    local beginIndex = (page - 1) * count
    local dataQuery = "select " .. field .. "from " ..table .. where .. " limit " .. beginIndex .. "," .. count

    local result = self:query(string.format(dataQuery,ndk.set_var.set_quote_sql_str(params)))
    local recordCount = self:query(string.format(countQuery,ndk.set_var.set_quote_sql_str(params)))[1]
    

    self:closeClient()
    local pc = math.ceil(recordCount["count"] / count)
    return {page={recordCount = recordCount["count"], pageCount =  pc, hasNext = (page < pc), hasPri = (page ~= 1)  }, data=result}
...
function MySQL:query_single(query)
...
-- @return number 自增索引值
function MySQL:add(tableName, entity, fields, replace)
    local values = {}
